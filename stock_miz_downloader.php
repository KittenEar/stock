...
$downloader = new StockMizDownloader();
...
while ($date <= $nowDate) {
    ...
    if (floor($progressPer % 10) == 0) {
        ...
        echo "currentProcess.innerHTML = '{$date->format(Common::MIZ_DATE_FORMAT)}';";
        echo "</script>";
    }

    // 株価ZIPファイルをダウンロード
    $ret = $downloader->downloadZipFile($date);

    // 株価データを追加
    $stockMiz->add($date);

    // echo "{$date->format(Common::MIZ_DATE_FORMAT)}</br>";
    ob_flush();
    flush();
